...
class Executor {
    ...
    public:
        ...
        // ReturnValue callfunc(string name, vector<string> argv): 함수 이름이 name인 함수를 argv를 매개변수로 하여금 실행하는 함수
        ReturnValue callfunc(const std::string& name, const std::vector<std::string>& argv);
};

std::string Executor::ReplaceAll(std::string &str, const std::string &from, const std::string &to) {
    size_t start_pos = 0;
...
std::string Executor::calculate(Type type, std::string value) {
    ...
    if (type == STRING) {
        ...
        if (find(input.begin(), input.end(), '\"') != input.end()) {
            ...
            for (char i : input) {
                if (i == '\"') is_string = !is_string;
                else if (!is_string && i == '+') {
                    ret += cache;
                    cache = "";
                }
                else if (is_string) cache += i;
            }
            if (!cache.empty()) ret += cache;  // +가 없는 식(Ex. "hello world") 경우
            ret = "\"" + ret + "\"";
            return ret;
        }
...
ReturnValue Executor::modifyString(Type type, std::string value) {
    int last_operator_idx = -1;  // 이전 연산자 인덱스
    std::string last_token;
    last_token = "";  // 연산자 전 나온 토큰
    ...
    for (int i = 0; i < value.length(); i++) {
        ...
        if (!is_nowchecking_string && !isdigit(value[i]) && find(operators, operators+5, tmp_tocmpr) == operators+5 && (value[i] != '(' && value[i] != ')') && value[i] != '.') {
            ...
            if (last_token.empty() || value[i-1] == '\"') {  // 문자열 검색 후 바로 다음에 오는 연산자일때(Ex. "hello"+"world")
                last_operator_idx = i;
            } else {
...
ErrorCode Executor::modifyVar(const std::string& tag, const std::string& value) {
    int idx = -1;
    for (int i = 0; i < tokens.size(); i++) {
        if (tokens[i] == tag) {
            idx = i; break;
...
ReturnValue Executor::callfunc(const std::string& name, const std::vector<std::string>& argv) {
    ...
    if (name == "out") {  // func<> out(string argv, ...) (...는 작동하지 않음)
        for (auto & i : argv) {
            ...
            switch (STRING) {
                case STRING:
                    argv_now = modifyString(STRING, i);
                    if (argv_now.value != special_token + "NOT_MATCHING_TYPE") break;
                case BOOL:  // same with int
                case INT:
                    argv_now = modifyString(INT, i);
                    if (argv_now.value != special_token + "NOT_MATCHING_TYPE") break;
                case DOUBLE:
                    argv_now = modifyString(DOUBLE, i);
                    if (argv_now.value != special_token + "NOT_MATCHING_TYPE") break;
                default:
                    return {NOT_MATCHING_TYPE, ""};
            }
