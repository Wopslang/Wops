...
// Deprecated
Expr ParseExpr(std::vector<String> tokens, int parsing_line) {
  Expr head({0, 0, 0}, Object("_", {}, {}, Variable("_", "", OPERATOR), 0, parsing_line, OK), parsing_line);

  if (tokens.size() == 0) ErrHandler().CallErr(parsing_line, BLANK_OPERAND, {});

  if (tokens.size() >= 3 && std::find(funcs.begin(), funcs.end(), tokens[0]) != funcs.end() && tokens[1] == "(" && tokens[tokens.size() - 1] == ")") {
    int level = 0;
    bool isSuitable = 1;
    ...
    if (isSuitable) {
      ...
      for (int idx = 2; idx < tokens.size() - 1; idx++) {
        if (tokens[idx] == ",") {
          if (parameter.size() == 0) ErrHandler().CallErr(parsing_line, BLANK_PARAMETER, {});
      ...
      if (parameter.size() != 0) head.AddChild(ParseExpr(parameter, parsing_line));
      return head;
    }
  }
  ...
  if (tokens[0] == "(" && tokens[tokens.size() - 1] == ")") {
    ...
    if (isSuitable) tokens = std::vector<String>(tokens.begin() + 1, tokens.end() - 1);
  }
  ...
  for (int idx = 0; idx < tokens.size(); idx++) {
    ...
    if (token == "||") {
      ...
      head.SetChildren({ParseExpr(std::vector<String>(tokens.begin(), tokens.begin() + idx), parsing_line), ParseExpr(std::vector<String>(tokens.begin() + idx + 1, tokens.end()), parsing_line)});
      return head;
    }
  }
  ...
  for (int idx = 0; idx < tokens.size(); idx++) {
    ...
    if (!isTarget[idx]) continue;
    if (token == "+") {
      if (idx != 0 && std::find(operators.begin(), operators.end(), tokens[idx - 1]) != operators.end()) continue;
      if (idx == tokens.size() - 1) ErrHandler().CallErr(parsing_line, OPERATION_ONLY_BINARY, {"+"});
      if (idx == 0) {
        if (tokens.size() != 2) ErrHandler().CallErr(parsing_line, NO_MATCHING_UNARY_OPERATION_FORM, {"+"});
        if (std::regex_match(tokens[1], std::regex("[0-9]+"))) {
          ...
          ErrHandler().CallErr(parsing_line, NO_OPERATION_MATCHING_TYPE_UNARY, {"+", "string"});
        } else {
          ErrHandler().CallErr(parsing_line, NO_OPERATION_MATCHING_TYPE_UNARY, {"+", "lvalue"});
        }
        return head;
      }
      head = Expr({0, 0, 0}, Object("_", {}, {}, Variable("_", "+", OPERATOR), 0, parsing_line), parsing_line);
    ...
    if (token == "-") {
      if (idx == tokens.size() - 1) ErrHandler().CallErr(parsing_line, OPERATION_ONLY_BINARY, {"-"});
      if (std::find(operators.begin(), operators.end(), tokens[idx - 1]) != operators.end()) continue;
      if (idx == 0) {
        if (tokens.size() != 2) ErrHandler().CallErr(parsing_line, NO_MATCHING_UNARY_OPERATION_FORM, {"-"});
        if (std::regex_match(tokens[1], std::regex("[0-9]+"))) {
          head = Expr({1, 0, 0}, Object("_", {}, {}, Variable("_", "-" + tokens[1], INT), 0, parsing_line), parsing_line);
  ...
  for (int idx = 0; idx < tokens.size(); idx++) {
    ...
    if (!isTarget[idx]) continue;
    if (token == "*") {
      if (idx == 0 || idx == tokens.size() - 1) ErrHandler().CallErr(parsing_line, NO_MATCHING_UNARY_OPERATION_FORM, {"*"});
      head = Expr({0, 0, 0}, Object("_", {}, {}, Variable("_", "*", OPERATOR), 0, parsing_line), parsing_line);
      head.SetChildren({ParseExpr(std::vector<String>(tokens.begin(), tokens.begin() + idx), parsing_line), ParseExpr(std::vector<String>(tokens.begin() + idx + 1, tokens.end()), parsing_line)});
      return head;
    }
  ...
  for (int idx = 0; idx < tokens.size(); idx++) {
    ...
    if (!isTarget[idx]) continue;
    if (token == "!") {
      if (idx == tokens.size() - 1) ErrHandler().CallErr(parsing_line, NO_IDENTIFIER_AFTER_OPERATOR, {"!"});
      head = Expr({0, 0, 0}, Object("_", {}, {}, Variable("_", "!", OPERATOR), 0, parsing_line), parsing_line);
      head.SetChildren({ParseExpr({tokens[idx + 1]}, parsing_line)});
      return head;
    }
  }
      ...
      switch (code[idx]) {
        ...
        case '>': {
          if (code.length() > idx + 1 && code[idx + 1] == '=') {
            ...
            token.resize(2);
            std::copy(code.begin() + idx, code.begin() + idx + 2, token.begin());
            token_table.push_back(token);
    ...
    for (int idx = parse_start; idx < token_table.size(); idx++) {
      ...
      switch (head.GetStmt()) {
        ...
        case IfStmt: {
          ...
          if (token_table[idx] == "?") {
            if (arg_idx == 1) {
              head.AddExpr(ParseExpr(token_storage, parsing_line));
              token_storage = {};
        ...
        // const variable declaration
        case ConstDel: {
          if (token_table[idx] == "if" || token_table[idx] == "?" || token_table[idx] == "for" || token_table[idx] == "continue" || token_table[idx] == "const" || token_table[idx] == "break" || token_table[idx] == "$" || token_table[idx] == ";") ErrHandler().CallErr(parsing_line, NO_MATCHING_SYNTAX_EXPRESSION, {});
          if (token_table[idx] == "=") {
            if (token_storage.size() == 2) {  // declaration
              head.AddArg(Object("_", {}, {}, {"_", token_storage[0]}));
              head.AddArg(Object("_", {}, {}, {"_", token_storage[1]}));
              token_storage = {};
